...
function guardar($conn) {
    ...
    $checkSql = "SELECT COUNT(*) as total FROM inst_etiqueta WHERE codigo_etiqueta = '$codigo'";
    $checkResult = sqlsrv_query($conn, $checkSql);
    $checkRow = sqlsrv_fetch_array($checkResult, SQLSRV_FETCH_ASSOC);
    
    if ($checkRow['total'] > 0) {
        echo "Error: Ya existe una etiqueta con ese cÃ³digo";
        return;
    }
    
    $sql = "INSERT INTO inst_etiqueta (codigo_etiqueta, nombre_etiqueta, id_exportadora) VALUES ('$codigo', '$nombre', " . ($id_exportadora ?: 'NULL') . ")";
...
function modificar($conn) {
    ...
    $sql = "UPDATE inst_etiqueta SET codigo_etiqueta = '$codigo', nombre_etiqueta = '$nombre', id_exportadora = " . ($id_exportadora ?: 'NULL') . " WHERE id_etiqueta = $id";
...
function eliminar($conn) {
    ...
    $sql = "DELETE FROM inst_etiqueta WHERE id_etiqueta = $id";
