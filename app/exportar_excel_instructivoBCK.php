...
foreach ($agrupadoPorPedido as $pedido => $filas) {
    foreach ($filas as $row) {
        ...
        if (!in_array($row['nombre_calibre'], $calibres)) {
            $calibres[] = $row['nombre_calibre'];
        }
    }
}

$pedidoCantidades = [];
$sqlCantidades = "SELECT numero_pedido, cantidad FROM inst_pedidos WHERE id_instructivo = ? AND version = ?";
...
sort($calibres); // Ordena los calibres alfabéticamente
...
$sheet->mergeCells('C1:N2')->setCellValue('C1', 'INSTRUCTIVO PLANTA ALMAHUE ');
$sheet->mergeCells('E3:L3')->setCellValue('E3', 'Exportadora: ' . $cabecera['nombre_exportadora']);
$sheet->mergeCells('E5:L5')->setCellValue('E5', 'Especie: ' . $cabecera['nombre_especie']);
$sheet->mergeCells('E6:L6')->setCellValue('E6', 'Variedad Real: ' . $cabecera['variedad']);
$sheet->mergeCells('E4:L4')->setCellValue('E4', 'Fecha: ' . $cabecera['fecha']->format('d-m-Y'));
$sheet->mergeCells('P3:Q3')->setCellValue('P3', 'Turno: ' . $cabecera['turno']);
$sheet->mergeCells('P4:Q4')->setCellValue('P4', 'Numero Instructivo: ' . $cabecera['id_instructivo']);
$sheet->mergeCells('P5:Q5')->setCellValue('P5', 'Versión: ' . $version);
$sheet->mergeCells('C8:N9')->setCellValue('C8', 'INFORMACION DE EMBALAJE ');
$sheet->getColumnDimension('C1')->setAutoSize(true);

// Estilo a encabezado visual
$sheet->getStyle('C1:N2')->applyFromArray($styleTitulo);
$sheet->getStyle('C8:N9')->applyFromArray($styleTitulo);
$sheet->getStyle('P3:Q5')->applyFromArray($borderStyle);
...
$encabezadosFijos = ['Pedido','Codigo Envase','Tipo Pallet'];
foreach ($encabezadosFijos as $enc) {
    $cell = Coordinate::stringFromColumnIndex($colIndex) . $filaEncabezados;
    $sheet->setCellValue($cell, $enc);
    $sheet->getStyle($cell)->applyFromArray($styleHeader);
...
foreach ($agrupadoPorPedido as $pedido => $filas) {
    ...
    foreach ($filas as $filaDatos) {
        ...
        $sheet->setCellValue(Coordinate::stringFromColumnIndex($col++) . $filaEncabezados, $filaDatos['embalaje_cod']);
...
$sheet->getStyle("A{$filaObs}:{$ultimaCol}{$filaObs}")->applyFromArray($styleYellowFill);
